...
def load_regular_data(path, *args, **kwargs):
    ...
    if path.endswith(".jsonl"):
        with jsonlines.open(path, 'r', *args, **kwargs) as jsonl_f:
            lst = [obj for obj in jsonl_f]
        return lst
    elif path.endswith(".json"):
        with open(path) as fin:
            lst = json.load(path, *args, **kwargs)
        return lst
    elif path.endswith(".csv") or path.endswith(".tsv"):
        df: pd.DataFrame = pd.read_csv(path, *args, **kwargs)
        return df.to_dict()
